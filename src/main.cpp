...
uint256 hashGenesisBlock("0x5f8e7e30b992ed80d83484c880257b66795f4b100a8ca9615cbbe2df0381949a");
...
const string strMessageMagic = "EurobitsCoin Signed Message:\n";
...
{
    // EurobitsCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // EurobitsCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "eurobits genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
	    ...
	    assert(block.hashMerkleRoot == uint256("0x6c3a59f67758271a28213793f6f95fddf4577e326b4f6fafe3b7908f131ccc85"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xe2, 0xce, 0xa3, 0x9b };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EurobitsCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EurobitsCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EurobitsCoinMiner(CWallet *pwallet)
{
    printf("EurobitsCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("EurobitsCoin-miner");
    ...
    try { loop {
        ...
        printf("Running EurobitsCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EurobitsCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EurobitsCoinMiner, pwallet));
}
