...
// Copyright (c) 2011-2012 EurobitsCoin Developers
...
void ThreadIRCSeed()
{
    ...
    RenameThread("EurobitsCoin-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #EurobitsCoinTEST0\r");
            Send(hSocket, "WHO #EurobitsCoinTEST0\r");
        } else {
            // randomly join #EurobitsCoin00-#EurobitsCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #EurobitsCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #EurobitsCoin%02d\r", channel_number).c_str());
        }
